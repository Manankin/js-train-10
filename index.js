...
function filterObjectsByKey(arr, key) {
  ...
  if (Array.isArray(arr)) {
    ...
    let result = arr.filter((item) => item.hasOwnProperty(key));
...
function checkAndPreventExtensions(obj) {
  ...
  if (typeof obj === 'object') {
    ...
    if (!Object.isFrozen(obj) || Object.isExtensible(obj)) {
...
function convertArrayToObj(arr) {
  ...
  if (Array.isArray(arr)) {
    ...
    //   } else {
    //     resultObject = Object.fromEntries(arr[i]);
    //   }
    // }
  ...
  // Застосовуємо метод Object.fromEntries() для створення об'єкта
    resultObject = Object.fromEntries(arr);
    return resultObject
  } else {
    return {}
  }
